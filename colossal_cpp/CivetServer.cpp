...
bool CivetHandler::handleGet(CivetServer *server, struct mg_connection *conn, int *status_code)
{
    UNUSED_PARAMETER(server);
    UNUSED_PARAMETER(conn);
    if (status_code)
...
bool CivetHandler::handlePost(CivetServer *server, struct mg_connection *conn, int *status_code)
{
    UNUSED_PARAMETER(server);
    UNUSED_PARAMETER(conn);
    if (status_code)
...
bool CivetHandler::handleHead(CivetServer *server, struct mg_connection *conn, int *status_code)
{
    UNUSED_PARAMETER(server);
    UNUSED_PARAMETER(conn);
    if (status_code)
...
bool CivetHandler::handlePut(CivetServer *server, struct mg_connection *conn, int *status_code)
{
    UNUSED_PARAMETER(server);
    UNUSED_PARAMETER(conn);
    if (status_code)
...
bool CivetHandler::handlePatch(CivetServer *server, struct mg_connection *conn, int *status_code)
{
    UNUSED_PARAMETER(server);
    UNUSED_PARAMETER(conn);
    if (status_code)
...
bool CivetHandler::handleDelete(CivetServer *server, struct mg_connection *conn, int *status_code)
{
    UNUSED_PARAMETER(server);
    UNUSED_PARAMETER(conn);
    if (status_code)
...
bool CivetHandler::handleOptions(CivetServer *server, struct mg_connection *conn, int *status_code)
{
    UNUSED_PARAMETER(server);
    UNUSED_PARAMETER(conn);
    if (status_code)
...
bool CivetWebSocketHandler::handleConnection(CivetServer *server, const struct mg_connection *conn)
{
    UNUSED_PARAMETER(server);
    UNUSED_PARAMETER(conn);
    return true;
...
void CivetWebSocketHandler::handleReadyState(CivetServer *server, struct mg_connection *conn)
{
    UNUSED_PARAMETER(server);
    UNUSED_PARAMETER(conn);
    return;
...
bool CivetWebSocketHandler::handleData(CivetServer *server, struct mg_connection *conn, int bits, char *data,
                                       size_t data_len)
{
    UNUSED_PARAMETER(server);
    UNUSED_PARAMETER(conn);
...
void CivetWebSocketHandler::handleClose(CivetServer *server, const struct mg_connection *conn)
{
    UNUSED_PARAMETER(server);
    UNUSED_PARAMETER(conn);
    return;
...
int CivetServer::webSocketConnectionHandler(const struct mg_connection *conn, void *cbdata)
{
    const struct mg_request_info *request_info = mg_get_request_info(conn);
    assert(request_info != NULL);
    CivetServer *me = (CivetServer *)(request_info->user_data);
...
void CivetServer::webSocketReadyHandler(struct mg_connection *conn, void *cbdata)
{
    const struct mg_request_info *request_info = mg_get_request_info(conn);
    assert(request_info != NULL);
    CivetServer *me = (CivetServer *)(request_info->user_data);
...
int CivetServer::webSocketDataHandler(struct mg_connection *conn, int bits, char *data, size_t data_len, void *cbdata)
{
    const struct mg_request_info *request_info = mg_get_request_info(conn);
    assert(request_info != NULL);
    CivetServer *me = (CivetServer *)(request_info->user_data);
...
void CivetServer::webSocketCloseHandler(const struct mg_connection *conn, void *cbdata)
{
    const struct mg_request_info *request_info = mg_get_request_info(conn);
    assert(request_info != NULL);
    CivetServer *me = (CivetServer *)(request_info->user_data);
    ...
    : context(0)
{
    ...
    if (context == NULL)
    {
        std::string exceptionMsg = "null context when constructing CivetServer. "
...
CivetServer::CivetServer(const std::vector<std::string> &options, const struct CivetCallbacks *_callbacks,
...
void CivetServer::addWebSocketHandler(const std::string &uri, CivetWebSocketHandler *handler)
{
    mg_set_websocket_handler(context, uri.c_str(), webSocketConnectionHandler, webSocketReadyHandler,
                             webSocketDataHandler, webSocketCloseHandler, handler);
}

void CivetServer::addAuthHandler(const std::string &uri, CivetAuthHandler *handler)
{
...
void CivetServer::urlDecode(const char *src, std::string &dst, bool is_form_url_encoded)
{
    urlDecode(src, strlen(src), dst, is_form_url_encoded);
}

void CivetServer::urlDecode(const char *src, size_t src_len, std::string &dst, bool is_form_url_encoded)
{
    // assign enough buffer
    std::vector<char> buf(src_len + 1);
    int r = mg_url_decode(src, static_cast<int>(src_len), &buf[0], static_cast<int>(buf.size()), is_form_url_encoded);
    if (r < 0)
    {
        // never reach here
...
bool CivetServer::getParam(struct mg_connection *conn, const char *name, std::string &dst, size_t occurrence)
{
    ...
    if (conobj.postData.empty())
    {
        ...
        for (;;)
        {
            ...
            try
            {
                ...
                else if ((r < 0) || ((conobj.postData.size() + r) > MAX_PARAM_BODY_LENGTH))
                {
    ...
    {
        get_param_success = getParam(queryString, strlen(queryString), name, dst, occurrence);
    }

    return get_param_success;
}

bool CivetServer::getParam(const char *data, size_t data_len, const char *name, std::string &dst, size_t occurrence)
{
    char buf[256];
...
std::vector<struct mg_server_port> CivetServer::getListeningPortsFull()
{
    ...
    for (;;)
    {
        int size = mg_get_server_ports(context, static_cast<int>(server_ports.size()), &server_ports[0]);
